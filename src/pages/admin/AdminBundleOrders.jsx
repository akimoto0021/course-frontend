  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }} onClick={onClose}>
      <div style={{ position:'relative', maxWidth:560, width:'100%' }} onClick={e => e.stopPropagation()}>
        <button onClick={onClose} style={{ position:'absolute', top:-16, right:-16, width:32, height:32, borderRadius:'50%', border:'none', background:'var(--brand)', color:'#fff', fontSize:16, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1 }}>×</button>
        {url?.match(/\.pdf$/i)
        ...
        <a href={url} target="_blank" rel="noreferrer" style={{ display:'block', textAlign:'center', marginTop:12, fontSize:12, color:'var(--text3)' }}>เปิดในแท็บใหม่ ↗</a>
      </div>
    </div>
  )
}
...
function DetailModal({ order, onClose }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:32, width:'100%', maxWidth:520, boxShadow:'0 24px 80px rgba(0,0,0,0.3)', maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ marginBottom:20, padding:'10px 14px', borderRadius:8, background:'var(--bg)', border:'1px solid var(--border)' }}>
          ...
          <span style={{ fontSize:13, fontWeight:700, color: statusColor[order.status] }}>{statusLabel[order.status] || order.status}</span>
          {order.rejected_reason && <div style={{ fontSize:12, color:'#dc2626', marginTop:6 }}>เหตุผล: {order.rejected_reason}</div>}
        </div>
        <div style={{ marginBottom:20 }}>
          ...
          {[['ชื่อ', order.name], ['อีเมล', order.email], ['เบอร์โทร', order.phone]].map(([l, v]) => (
...
export default function AdminBundleOrders() {
  ...
  // SSE — reload อัตโนมัติเมื่อมี order ใหม่
  useEffect(() => {
    const token = localStorage.getItem('accessToken')
    if (!token) return
    const es = new EventSource(`${API}/bundle/admin-stream?token=${token}`)
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, margin:0 }}>Bundle Orders</h2>
        {newOrderAlert && (
          <span onClick={() => setNewOrderAlert(false)}
            style={{ fontSize:12, background:'#dcfce7', color:'#16a34a', padding:'4px 10px', borderRadius:20, cursor:'pointer', fontWeight:600 }}>
            🔔 มี Order ใหม่! คลิกเพื่อปิด
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap' }}>
        {STATUS_TABS.map(([k, l]) => (
          <button key={k} onClick={() => { setFilter(k); setNewOrderAlert(false) }}
        ...
        {loading
          ...
          : (
            <table className="data-table">
              <thead>
                <tr><th>ลูกค้า</th><th>Bundle</th><th>ยอด</th><th>สลิป</th><th>วันที่</th><th>การดำเนินการ</th></tr>
              </thead>
              <tbody>
                ...
                {orders.map(o => (
                  <tr key={o.id}>
                    ...
                    <td style={{ fontSize: 13, cursor:'pointer' }} onClick={() => setDetailOrder(o)}>{BUNDLE_LABELS[o.bundle_type] || o.bundle_type}</td>
                    <td style={{ fontWeight: 600, cursor:'pointer' }} onClick={() => setDetailOrder(o)}>฿{Number(o.amount).toLocaleString()}</td>
                    <td>
                      {o.slip_url
                        ? <button onClick={() => setSlipUrl(o.slip_url)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', fontSize: 11, cursor: 'pointer', color: 'var(--brand)' }}>🖼 ดูสลิป</button>
                        : <span style={{ fontSize: 12, color: 'var(--text3)' }}>ยังไม่มี</span>
                      }
                    </td>
                    <td style={{ fontSize: 12, color: 'var(--text2)', cursor:'pointer' }} onClick={() => setDetailOrder(o)}>{new Date(o.created_at).toLocaleDateString('th-TH')}</td>
                    <td>
